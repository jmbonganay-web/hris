...
const datasets = new Set<ReportExportDataset>([
  "daily", "employee_summary", "exceptions", "overtime_holiday",
  "leave_balances", "leave_usage", "leave_conflicts",
]);
...
async function loadCsv(filters: ReportFilters, dataset: ReportExportDataset): Promise<{ csv: string; count: number }> {
  switch (dataset) {
    case "daily": {
    ...
    case "leave_balances": {
      ...
      return { csv: leaveBalanceCsv(result.rows), count: result.rows.length };
    }
    case "leave_usage": {
      const result = await getLeaveUsageReport(filters, true);
      return { csv: leaveUsageCsv(result.rows), count: result.rows.length };
    }
    case "leave_conflicts": {
      const result = await getLeaveConflictReport(filters, true);
      return { csv: leaveConflictCsv(result.rows), count: result.rows.length };
    }
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!datasets.has(rawDataset as ReportExportDataset)) return safeJsonError("The selected export dataset is invalid.", 400);
    const dataset = rawDataset as ReportExportDataset;
    const generated = await loadCsv(filters, dataset);
    await recordReportExportAudit({ dataset, format: "csv", filters, rowCount: generated.count });

    return new Response(generated.csv, {
      status: 200,
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="${csvFilename(dataset, filters.startDate, filters.endDate)}"`,
        "Cache-Control": "private, no-store, max-age=0",
        Pragma: "no-cache",
        "X-Content-Type-Options": "nosniff",
      },
    ...
    console.error("Report CSV export failed.");
